...
/// \brief Conversion from DWORD to UnicodeString
/// \param[in] local_Input Input value to be converted
/// \return Output value
cnv::unistr		DWORDtoUSTRING  	(cnv::dword local_Input)
{
	std::stringstream StrBuffer;
	StrBuffer << local_Input;
	std::string local_Output = StrBuffer.str();
...
/// \brief Conversion from const char* to UnicodeString
/// \param[in] local_Input Input value to be converted
/// \return Output value
cnv::unistr		CONSTCHARtoUSTRING	(const char* local_Input)
{
	std::string StrBuffer(local_Input, strlen(local_Input));
	cnv::unistr local_Output = StrBuffer.data();
	return local_Output;
