/// \file
/// \brief Pak file module
/// \author SpinelDusk
/// \details Pak file processing class module
...
#define SendAlert(a,b) SendAlert(__FILE__,__FUNCSIG__,__LINE__,a,2,b)

/// \brief Clearing instance fields (pack instance)
void PakPack::Clear()
{
	FileSignature = 0xFEEDFACE;
	AmountEntries = 0;
	DataStartOffset = 0;
...
/// \brief Pak-file packing control function
...
int PakPack::Pack(char* local_Input, char* local_Output)
{
	...
	//Create an output file, the future pak-archive
...
/// \return Error code: 0 - no errors, 1 - operation error
int PakPack::ReadOpenList(ifstream& List)
{
	while(List)
	{
		char PackCommand;
...
int PakPack::AddCompressedFile(cnv::unsint Count)
{
	ifstream AddedFile;
...
/// \param[in] Count Ordinal number of the file being packed
/// \return Error code: 0 - no errors, 1 - operation error
int PakPack::AddUncompressedFile(cnv::unsint Count)
{
	ifstream AddedFile;
...
/// \param[in] Count Ordinal number of the file being packed
/// \return Error code: 0 - no errors, 1 - operation error
int PakPack::AddEmptedFile(cnv::unsint Count)
{
	//Saving offset and compression flag into receive file
	std::streamoff FtellRespond = OpenReceiver.tellp();
	if(FtellRespond != -1)
...
void PakPack::FillHead()
{
	...
	for(int j = 0; j < 4; j++)
	{
		OpenReceiver << static_cast<cnv::byte>(FileSignature >> 8 * j);
	}
...
/// \return Error code: 0 - no errors, 1 - operation error
int PakUnpack::Load(char* local_Input)
{
	//Clearing the instance fields
	Clear();
	...
	//Get file signature 
	cnv::dword TemporarySignature = 0;
	if(DwordRead(OpenArchive, TemporarySignature))
	{
		OpenArchive.close();
		return 1;
	}
	if(TemporarySignature != GetFileSignature())
	{
		OpenArchive.close();
		SendAlert(17, Input);//The signatures don't match. The file is corrupted or is not an archive.
		return 1;
	}
...
/// \return Error code: 0 - no errors, 1 - operation error
int PakUnpack::ExtractFromArchive(cnv::unsint Count)
{
	//Checking for loading the archive into memory
	if(ArchivePath.empty())
	{
		SendAlert(18, nullptr);//Archive not loaded into memory.
		return 1;
	}
	...
	{
		SendAlert(19, nullptr);//Violating boundaries of the table of contents (Attempted to read outside the list limits).
		return 1;
    }
	...
	{
		SendAlert(20, ArchivePath);//Attempt to unpack empty entry.
		return 1;
	}
	...
	if(!OpenArchive.is_open())
	{
		SendAlert(21, ArchivePath);//Can't open input archive file. File does not exist or is locked. (Or the file path contains Unicode characters.)
		return 1;
	}
	...
	{
		OpenArchive.close();
		SendAlert(22, DirectoryForExtract);//Directory does not exist.
		return 1;
	}
	...
	{
		OpenArchive.close();
		SendAlert(23, Receiver);//The file can't be opened or does not exist.
		return 1;
    }
	...
	{
		SendAlert(24, Receiver);//The created file does not exist or has size of zero.
		return 1;
	}
	return 0;
}
...
/// \return Error code: 0 - no errors, 1 - operation error
int PakUnpack::UnpackEntireArchive()
{
	for(cnv::dword i = 0; i < AmountEntries; i++)
	{
...
/// \return Error code: 0 - no errors, 1 - operation error
int PakUnpack::CreateListFile()
{
	//Checking for loading the archive into memory
	if(ArchivePath.empty())
	{
		SendAlert(25, nullptr);//Archive not loaded into memory.
		return 1;
	}
	...
	else
	{
		...
		SendAlert(26, ListFileName);//Attempt to create list failed. File does not exist or is locked. (Or the file path contains Unicode characters.)
		return 1;
	}
	return 0;
}
...
int DwordRead(ifstream& local_Input, std::vector<cnv::dword>& local_Output, cnv::unsint Amount)
{
	...
	else
	{
		...
		if(!local_Input.good())
		{
			if(local_Input.eof())
			{
				SendAlert(27, nullptr);//The file data ended before data block was read. The file is corrupted.
				return 1;
			}
			else if(local_Input.fail())
			{
				SendAlert(28, nullptr);//ReadStream - Logical error on input operation.
				return 1;
			}
			else if(local_Input.bad())
			{
				SendAlert(29, nullptr);//ReadStream - Read error on input operation.
				return 1;
			}
		}
...
int StreamRead(ifstream& InputStream, char* Buffer, int ReadSize)
{
	...
	if(!InputStream.good())
	{
		...
		{
			SendAlert(34, nullptr);//StreamRead - Logical error on input operation.
			return 1;
		}
		else if(InputStream.bad())
		{
			SendAlert(35, nullptr);//StreamRead - Read error on input operation.
			return 1;
		}
	}
	return 0;
